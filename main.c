...
static void init_mmap(struct webcam_info * cam_info)
{
    ...
    if (-1 == ioctl(cam_info->fd, VIDIOC_REQBUFS, &req)) {
        if (EINVAL == errno) {
...
static void init_device(struct webcam_info *cam_info) {
    ...
    if (-1 == ioctl(cam_info->fd, VIDIOC_QUERYCAP, &cap)) {
        if (EINVAL == errno) {
    ...
    if (0 == ioctl(cam_info->fd, VIDIOC_CROPCAP, &cropcap)) {
...
static int read_frame(struct webcam_info *cam_info) {
    ...
    if (output != NULL) {
        fwrite(cam_info->buffers[buf.index].start,buf.bytesused , 1, output);
        fclose(output);
    }
    else {
